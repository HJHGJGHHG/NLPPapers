...
from models import Bert_SST2

logger = logging.getLogger(__name__)
logger.setLevel(level=logging.INFO)
handler = logging.FileHandler("bert_sst2_log.txt")
...
def main(args):
    ...
    model = Bert_SST2.from_pretrained(args.model_name_or_path, num_labels=2)
    model.to(device)
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for batch in metric_logger.log_every(train_data_loader, args.print_freq, header):
            ...
            with torch.cuda.amp.autocast(enabled=scaler is not None):
                outputs = model(**batch)
                loss = criterion(
                    outputs[0].reshape(-1, model.num_labels), labels.reshape(-1))
...
def get_args_parser(add_help=True):
    ...
    parser.add_argument("--model_name_or_path", default="/root/autodl-tmp/models/bert_base_uncased",
    ...
    parser.add_argument("--output_dir", default='bert_sst2', help="path where to save")
